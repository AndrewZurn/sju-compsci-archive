...
public class FiveRecSchoolsController {
    ...
    private void findDifference() {
        for (University candidate : allU) {
            Double distance = 0.0;
            if (!u.getState().toLowerCase().equals(candidate.getState().toLowerCase())) {
                distance++;
            }

            if (!u.getLocation().toLowerCase().equals(candidate.getLocation().toLowerCase())) {
                distance++;
            }

            if (!u.getControl().toLowerCase().equals(candidate.getControl().toLowerCase())) {
                distance++;
            }
            ...
            //System.out.println(candidate.getSchool() + "|"+distance);
    ...
    private void findFiveClosest() {
        ...
        for (Double score : allScore) {
            Double temp = score;
            while(ts.contains(temp)){//to prevent duplicate scores from being added
                temp=temp+0.000001;
            }            
            ts.add(temp);
    ...
    public static void main(String[] args) {
        System.out.println("***Here is Use Case #8: View Additional School Info and Five Recommended Schools***");
        University univ = DatabaseController.getAllUniversities().get(0);
        FiveRecSchoolsController frsc = new FiveRecSchoolsController(univ);
        System.out.println("\nThe school to compare to is: " + univ + "\n");
        System.out.println("The five most similar schools are: ");
