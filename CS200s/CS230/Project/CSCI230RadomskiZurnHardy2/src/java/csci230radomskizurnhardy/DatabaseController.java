...
//        }
    ...
    public static University getSpecificUniversity(String schoolName){
        University u = null;
        ArrayList<University> allUniversities = getAllUniversities();
        for(University uni: allUniversities){
            if(uni.getSchool().toLowerCase().equals(schoolName.toLowerCase())){
                u = uni;
            }
        }
        return u;
...
//     }
    ...
    public static ArrayList<User> getAllUsers() {
        ...
        for (int i = 0; i < temp.length; i++) {
            ...
            char tempG = temp[i][6].charAt(0);
            User tempU = new User(tempB, tempC, tempD, tempE, tempF, tempG);
            allUsers.add(tempU);
//            if(tempD.equals("CCBUsername2")){
//                System.out.println("ID: " + temp[i][0]);
//            }
            }
            catch(Exception e){
    ...
    /**
     * When called, this method returns an integer referring to the ID number
     * of the specified User. If the parameter username matches one in the 
     * database, it will be returned. Otherwise, a -1 is returned.
     *
     * @return Returns the ID number of the specified User.
     */
    public static int getID(String username){
        ...
        for (int i = 0; i < temp.length; i++) {
             try {
                int tempA = new Integer(temp[i][0]);
                String tempD = temp[i][3];
                if (tempD.equals(username)) {
                    return tempA;
                }
            } catch (Exception e) {
            }
    ...
    /**
     *  WIll return a specific user from the database that has the identification number ID.
     * @param Id
    ...
    public static void main(String args[]) {
        //System.out.println(getSpecificUniversity("AdELPHI"));
