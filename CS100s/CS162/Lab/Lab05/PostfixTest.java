...
public class PostfixTest extends TestCase {
  ...
  public void testBadExpressions() {
    ...
    try {
      pf1.infixToPostfix("(x+y))");
      fail("Something is wrong: Missing left parenthesis.");
    }
    catch(Exception e) { //Note we print out the exception this time
                         //You can see the exception in the Interactions tab
      System.out.println(e);
    ...
    try {
      pf1.infixToPostfix("(x+y");
      fail("Something is wrong: Missing right parenthesis.");
    }
    catch(Exception e) { //Note we print out the exception this time
                         //You can see the exception in the Interactions tab
      System.out.println(e);
    ...
    try {
      pf1.infixToPostfix("y&x");
      fail("Something is wrong: Bad/Illegal Character");
    }
    catch(Exception e) { //Note we print out the exception this time
                         //You can see the exception in the Interactions tab
      System.out.println(e);
  ...
  /**
   * this method will test the infixToPostfix method of class Postfix
   */
  public void testInfix(){
    assertEquals("It should be abc^^d* in postFix", pf1.infixToPostfix("a^b^c*d"),"abc^^d*");
    assertEquals("It should be ab*cd^+xyqr-*+/ in postFix", pf1.infixToPostfix("(a*b+c^d)/(x+y*(q-r))"),"ab*cd^+xyqr-*+/");
    assertEquals("It should be ab+cd*e/- in postFix", pf1.infixToPostfix("a+b-c*d/e"),"ab+cd*e/-");
  }
