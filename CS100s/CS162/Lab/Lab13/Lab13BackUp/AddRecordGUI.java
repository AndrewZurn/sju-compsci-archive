...
public class AddRecordGUI extends JFrame implements ActionListener {
  ...
  public void actionPerformed (ActionEvent evt) {
    ...
    if (source == addTrack) {
      addTrackGUI = new AddTrackGUI();
      for(Track t: trackList){
        trackMap.put(t.getTitle(), t);
      }
    }
    else if (source == addArtist) {
      addArtistGUI = new AddArtistGUI();
      artistMap.put(addArtistGUI.getArtistName(), addArtistGUI.returnArtist());
    }
    else {
      ...
      ArrayList<Track> trackList = addTrackGUI.getTrackList();
      Record record = new Record(recordTitle, addArtistGUI.getArtistName(), recordGenre, recordProducer, recordReleaseDate,
